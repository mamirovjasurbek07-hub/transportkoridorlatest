...
Public xaritaning default `map_mode=posts` rejimi corridor geometriyasini umuman yubormaydi. `map_mode=top5` faqat tashuv hajmi eng katta beshta post yo‘nalishini, `corridor=CODE` esa faqat tanlangan corridorni qaytaradi. Corridor katalogi `/corridors?include_geometry=false` bilan waypoint/geometriyasiz olinadi. Geometriyalar N+1 so‘rov o‘rniga bitta `ST_AsGeoJSON ... WHERE id IN (...)` batch so‘rovda olinadi; FastAPI 1 KB dan katta javoblarni GZip qiladi.
...
Demo seed `DEMO_V4` manbasi bilan 10 000 ta deklaratsiya va 52 ta shablon yo‘lak yaratadi. Ular Qirg‘iziston–Afg‘onistonning 4 ta va Qozog‘iston–Turkmanistonning 8 ta post kombinatsiyasi, Xitoydan g‘arb/janub tranziti, Rossiya, Kavkaz, Janubiy Osiyo hamda O‘zbekiston–Xitoy/Yevropa/Turkiya/Afg‘oniston yo‘nalishlarini qamrab oladi. Qo‘lda yozilgan eski `verified-osrm-seed-v2/v3/v4` LineStringlar router natijasi hisoblanmaydi: startup ularni `seed-routing-pending-v5` holatiga o‘tkazib geometriyasini o‘chiradi. Background vazifa waypointlarni OSRM Route API’ga tartib bilan yuborib faqat qaytgan `full` GeoJSON geometriyasini `osrm-router` sifatida saqlaydi. Xato bo‘lgan corridor `REVIEW` bo‘lib qoladi va public xaritada to‘g‘ri chiziq bilan almashtirilmaydi.

Yangi corridor konstruktori davlat tanlanganda mamlakat koordinatasini boshlang‘ich gateway sifatida qo‘yadi. Admin markerning o‘zini aniq ombor yoki logistika nuqtasiga sudrab ko‘chirishi, kirish/chiqish postlarini tanlashi va istalgancha VIA nuqta qo‘shishi mumkin. Chiziq ustiga bosilgan yangi VIA eng yaqin waypoint segmentiga qo‘yiladi. Nuqtalar o‘zgarganda preview 700 ms debounce bilan OSRM orqali avtomatik hisoblanadi; eski chiziq yangi javob kelguncha yo‘qolmaydi va xarita faqat constructor birinchi ochilganda corridor chegarasiga avtomatik moslanadi. Preview tasdiqlangandan keyin gateway, post, VIA tartibi, route geometriyasi, masofa, vaqt va router provayderi Supabase/PostGIS bazasida saqlanadi.
