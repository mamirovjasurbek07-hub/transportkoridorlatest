  ...
  return (
    <section className="filter-panel panel" aria-label="Tranzit ma'lumotlarini filtrlash">
      ...
      <div className="filter-grid">
        <label><span>Tashuv boshlangan davlat</span><div className="input-wrap"><Search size={16} /><select value={draft.origin} onChange={(e) => update('origin', e.target.value)}><option value="">Barcha davlatlar</option>{countries.map((c) => <option key={c.alpha2} value={c.alpha2}>{c.flag} {c.name} · {c.alpha3}</option>)}</select></div></label>
        <label><span>Tashuv tugallangan davlat</span><div className="input-wrap"><Search size={16} /><select value={draft.destination} onChange={(e) => update('destination', e.target.value)}><option value="">Barcha davlatlar</option>{countries.map((c) => <option key={c.alpha2} value={c.alpha2}>{c.flag} {c.name} · {c.alpha3}</option>)}</select></div></label>
        ...
        <details className="advanced-filters"><summary>Qo'shimcha filtrlar</summary><div className="advanced-grid">
          ...
          <label><span>Korridor</span><select value={draft.corridor} onChange={(e) => update('corridor', e.target.value)}><option value="">Barchasi</option>{corridors.map((c) => <option key={c.id} value={c.code}>{c.name}</option>)}</select></label>
        </div></details>
      </div>
